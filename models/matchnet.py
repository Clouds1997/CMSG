...
from models.layers_batch import AttentionModule, TenorNetworkModule
...
class PointNetVlad(nn.Module):
    def __init__(self, num_points=2500, global_feat=True, feature_transform=False, max_pool=True, output_dim=1024):
        ...
        self.args = options_matchnet.Options()
        self.args.filters_3 = 256
...
class ImgNetVlad(nn.Module):
    def __init__(self, num_points=80, global_feat=True, feature_transform=False, max_pool=True, output_dim=1024):
        super(ImgNetVlad, self).__init__()
        self.args = options_matchnet.Options()
        self.args.filters_3 = 256
        self.res_net = networks_img.ImageEncoder_Base((self.args))
        self.net_vlad = NetVLADLoupe(feature_size=512, max_samples=80, cluster_size=16,
                                     output_dim=output_dim, gating=True, add_batch_norm=True,
                                     is_training=True)
    ...
    def forward(self, x):
        # x1 = [B, 3, n] ===> [B, 1, n ,3]
        # x = x.unsqueeze(1).permute(0,1,3,2)
        img_s16_feature_map, img_s32_feature_map, img_global_feature = self.res_net(x)
        # print(img_s32_feature_map.shape)
        x = self.net_vlad(img_s32_feature_map)
        x = x.unsqueeze(2)
...
class matchnet(nn.Module):
    def __init__(self, num_points=2500):
        ...
        self.args = options_matchnet.Options()
        self.args.filters_3 = 256
        self.pointnetvlad = PointNetVlad(global_feat=True, feature_transform=True, max_pool=False,
                                            output_dim=256, num_points=num_points)
        self.img_netvlad = ImgNetVlad(global_feat=True, feature_transform=True, max_pool=False,
                                            output_dim=256, num_points=81920)
    ...
    def forward(self, x1, x2):
        ...
        # x2 = x2.unsqueeze(1).permute(0,1,3,2)
        x1 = self.pointnetvlad(x1)

        # print(x1.shape)


        x2 = self.img_netvlad(x2)
...
if __name__ == '__main__':
    num_points = 5120
    # 点云数据
    sim_data1 = torch.rand(2,3,num_points)
    sim_data1 = sim_data1.cuda()

    # 图像数据
    sim_data2 = torch.rand(2,3,160,512)
    sim_data2 = sim_data2.cuda()

    pnv = matchnet(num_points=num_points).cuda()
    pnv.train()
    out3 = pnv(sim_data1,sim_data2)
    print('pnv', out3)